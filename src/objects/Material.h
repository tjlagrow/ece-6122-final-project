...
public:
	...
	const glm::vec3 &getEmission() const { return m_Ke; }
	const float &getShininess() const { return m_shininess; }
	const float &getRefraction() const { return m_Ni; }
	const float &getSpecularExponent() const { return m_Ns; }
	const float &getDissolveFactor() const { return m_d; }
	const unsigned int &getIllumination() const { return m_illum; }
	const int &getShadingModel() const { return m_shadingModel; }
	const int &getBlendFunc() const { return m_blendFunc; }
	const int &getWireframe() const { return m_enableWireframe; }
