...
void threading_func(Vector3f *p, const unsigned &width, const unsigned &threaID, const int &numPerThread, const int &extra,
					const float &invertedWidth, const float &invertedHeight, const float &angle, const std::vector<Sphere> &spheres, const float &aspectRatio, const glm::vec3 &CamPos){
...
//	spheres.push_back(wall);
	...
	if(!parallel) {
		...
		int numPerThread = m_height / NUMTHREADS;
		int extra = m_height % NUMTHREADS;
		for (unsigned i = 0; i < NUMTHREADS; ++i) {
			threads[i] = std::thread(threading_func, pixel + (i * numPerThread * m_width), m_width, i, numPerThread, extra,
